...
DEFAULT_CURL_IMPERSONATE = "chrome"
...
class Account:
    ...
    :param request_max_retries: Максимальное количество повторных попыток отправки запроса, если была обнаружена антибот-защита сайта.
    ...
    def __init__(
        ...
        self.__session_warmed_up = False
    ...
    def _get_user_agent(self) -> str:
        return self.user_agent.strip() if self.user_agent else DEFAULT_USER_AGENT
    ...
    def _apply_auth_cookies(self) -> None:
        if not self.token:
            return
        for domain in ("playerok.com", ".playerok.com"):
            self.__curl_session.cookies.set("token", self.token, domain=domain, path="/")

    def _build_cookie_header(self) -> str:
        cookie_map = {}
        for cookie in self.__curl_session.cookies.jar:
            cookie_map[cookie.name] = cookie.value
        if self.token:
            cookie_map["token"] = self.token
        return "; ".join(f"{key}={value}" for key, value in cookie_map.items())

    def _warm_up_web_session(self, force: bool = False) -> None:
        if self.__session_warmed_up and not force:
            return
        try:
            self.__curl_session.get(
                url=f"{self.base_url}/",
                headers={
            ...
            self.__session_warmed_up = True

    def _is_protection_response(self, response) -> bool:
        text = getattr(response, "text", "") or ""
        server = str(getattr(response, "headers", {}).get("server", "")).lower()
        set_cookie = str(getattr(response, "headers", {}).get("set-cookie", "")).lower()
        ...
        if any(signature in text for signature in protection_signatures):
            return True
        if "__ddg" in set_cookie or "ddos-guard" in server:
            return response.status_code in (403, 429, 503)
        return False

    def _get_protection_name(self, response) -> str:
        server = str(getattr(response, "headers", {}).get("server", "")).lower()
        set_cookie = str(getattr(response, "headers", {}).get("set-cookie", "")).lower()
        text = getattr(response, "text", "") or ""
        if "ddos-guard" in server or "__ddg" in set_cookie or "DDoS-Guard" in text:
            return "DDoS-Guard"
        return "Cloudflare"
    ...
    def request(self, method: Literal["get", "post"], url: str, headers: dict[str, str], 
        ...
        _headers = {
            ...
            "user-agent": self._get_user_agent(),
            "x-gql-op": operation_name or "viewer",
            "x-timezone-offset": self._get_timezone_offset()
        }
        request_headers = _headers.copy()
        ...
        def make_req():
            if method == "get":
                ...
                if files:
                    upload_headers = request_headers.copy()
                    upload_headers["cookie"] = self._build_cookie_header()
        ...
        self._warm_up_web_session()
        max_retries = max(1, int(self.request_max_retries))
        for attempt in range(max_retries):
            resp = make_req()
            if not self._is_protection_response(resp):
                break
            delay = min(30.0, 2.0 * (2 ** attempt))
            protection_name = self._get_protection_name(resp)
            self.__logger.error(
                f"{protection_name} detected, пробую отправить запрос снова через {delay} секунд"
            )
            if attempt >= 1:
                self._refresh_clients()
            self._warm_up_web_session(force=True)
            time.sleep(delay)
        else:
