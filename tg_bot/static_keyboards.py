...
def SETTINGS_SECTIONS_2() -> K:
    return K() \
        ...
        .add(B(_("gl_next"), callback_data=CBT.MAIN3)) \
        .add(B(_("gl_back"), callback_data=CBT.MAIN))


def SETTINGS_SECTIONS_3() -> K:
    return K() \
        .add(B(_("mm_lot_map"), callback_data=f"{CBT.CATEGORY}:map")) \
        .add(B(_("mm_notify_center"), callback_data=f"{CBT.CATEGORY}:nc")) \
        .add(B(_("mm_quick_actions"), callback_data=f"{CBT.CATEGORY}:qa")) \
        .add(B(_("mm_sales_stats"), callback_data=f"{CBT.CATEGORY}:stats")) \
        .add(B(_("mm_smart_replies"), callback_data=f"{CBT.CATEGORY}:sar")) \
        .add(B(_("mm_mass_lots"), callback_data=f"{CBT.CATEGORY}:lots")) \
        .add(B(_("mm_safe_mode"), callback_data=f"{CBT.CATEGORY}:safe")) \
        .add(B(_("mm_scheduler"), callback_data=f"{CBT.CATEGORY}:sched")) \
        .add(B(_("mm_backups"), callback_data=f"{CBT.CATEGORY}:bkp")) \
