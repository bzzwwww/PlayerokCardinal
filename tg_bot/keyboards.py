...
def lot_map_settings() -> K:
    return K() \
        .row(B(_("gl_refresh"), None, f"{CBT.CATEGORY}:map"),
             B(_("fc_clear_lot_map"), None, CBT.CLEAR_LOT_MAP)) \
        .add(B(_("gl_back"), None, CBT.MAIN3))


def notification_center_settings(chat_id: int) -> K:
    return K() \
        .add(B(_("fc_open_chat_notifications"), None, f"{CBT.CATEGORY}:tg")) \
        .add(B(_("fc_open_announcements"), None, CBT.OPEN_ANNOUNCEMENTS)) \
        .add(B(_("fc_test_notification"), None, CBT.FEATURE_TEST_NOTIFICATION)) \
        .add(B(_("gl_back"), None, CBT.MAIN3))


def quick_actions_settings() -> K:
    return K() \
        .row(B(_("fc_to_stats"), None, f"{CBT.CATEGORY}:stats"),
             B(_("fc_to_autoresponse"), None, f"{CBT.CATEGORY}:sar")) \
        .add(B(_("gl_back"), None, CBT.MAIN3))


def sales_stats_settings() -> K:
    return K() \
        .row(B(_("gl_refresh"), None, f"{CBT.CATEGORY}:stats"),
             B(_("fc_reset_stats"), None, CBT.RESET_FEATURE_STATS)) \
        .add(B(_("gl_back"), None, CBT.MAIN3))


def smart_replies_settings() -> K:
    return K() \
        .row(B(_("fc_to_autoresponse"), None, f"{CBT.CATEGORY}:ar"),
             B(_("fc_to_templates"), None, f"{CBT.TMPLT_LIST}:0")) \
        .add(B(_("gl_back"), None, CBT.MAIN3))


def mass_lots_settings() -> K:
    return K() \
        .row(B(_("fc_to_lots"), None, f"{CBT.AD_LOTS_LIST}:0"),
             B(_("fc_to_goods_files"), None, f"{CBT.PRODUCTS_FILES_LIST}:0")) \
        .add(B(_("fc_refresh_lots"), None, "update_funpay_lots:0")) \
        .add(B(_("gl_back"), None, CBT.MAIN3))


def safe_mode_settings(c: Cardinal) -> K:
    status = bool_to_text(feature_tools.safe_mode_enabled(c), on="🟢", off="🔴")
    return K() \
        .add(B(_("fc_toggle_safe_mode", status), None, CBT.TOGGLE_SAFE_MODE)) \
        .add(B(_("gl_back"), None, CBT.MAIN3))


def scheduler_settings() -> K:
    return K() \
        .row(B(_("fc_scheduler_set_time"), None, CBT.SET_BACKUP_SCHEDULE),
             B(_("fc_scheduler_disable"), None, CBT.DISABLE_BACKUP_SCHEDULE)) \
        .add(B(_("fc_backup_create"), None, CBT.FEATURE_CREATE_BACKUP)) \
        .add(B(_("gl_back"), None, CBT.MAIN3))


def backups_settings() -> K:
    return K() \
        .row(B(_("fc_backup_create"), None, CBT.FEATURE_CREATE_BACKUP),
             B(_("fc_backup_get"), None, CBT.FEATURE_GET_BACKUP)) \
        .add(B(_("fc_backup_upload"), None, CBT.FEATURE_UPLOAD_BACKUP)) \
        .add(B(_("gl_back"), None, CBT.MAIN3))
...
def LINKS_KB(language: None | str = None) -> K:
    return K().add(B(_("mm_plugins", language=language), url="https://t.me/bzwwplugins_bot")) \
        .add(B(_("lnk_chat", language=language), url="https://t.me/bzwwplugins_bot"))
